...
public class PerformanceDataGenerator {
    ...
    private List<Artist> generateOrGetArtists() {
    ...
    private List<Location> generateOrGetLocations() throws JsonProcessingException  {
        Faker f = new Faker();
    ...
    private void generatePerformances() throws JsonProcessingException {
        List<Artist> artists = generateOrGetArtists();
        List<Location> locations = generateOrGetLocations();
        Faker f = new Faker();
        if (!eventRepository.findAll().isEmpty()) {
            ...
            for (int i = 0; i < NUMBER_OF_EVENTS_TO_GENERATE; i++) {
                Artist artist = artists.get(rand.nextInt(artists.size()));

                Event event = new Event();
                event.setTitle(f.book().title());
                event.setDescription(f.hitchhikersGuideToTheGalaxy().quote());
                event.setCategory(CategoryEnum.values()[rand.nextInt(CategoryEnum.values().length)]);
                ...
                performance.setLocation(locations.get(rand.nextInt(locations.size())));

                Performance performance2 = new Performance();
                performance2.setDateTime(OffsetDateTime.now().plusDays(i+1).plusHours(i));
                performance2.setEvent(event);
                performance2.setLocation(locations.get(rand.nextInt(locations.size())));
